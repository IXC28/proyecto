...
const storage = multer.diskStorage({
  ...
  paquetesRouter.get('/', async (request, response) => {
    ...
    return response.status(200).json({logiado ,rol, paquete});

});
paquetesRouter.delete('/:id', async (request, response) => {
    try {
      ...
      await Paquete.findByIdAndDelete(request.params.id);
